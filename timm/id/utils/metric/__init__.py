...
def accuracy_pos_neg(output, target, split_pos_neg=True):
    ...
    Args:
        ...
        split_pos_neg (bool): Whether to calculate the accuracy of positive and negative classes additionally.

    Returns:
        Top-1 accuracies (have 3 values).
    """
    with torch.no_grad():
        batch_size = target.size(0)
        ...
        res = {}
        correct_all = correct[:1].flatten().s(dtype=torch.float32)
        res["all"] = correct_all * (100.0 / batch_size)
        if split_pos_neg:
            correct_pos = (pred == 1) * (targ == 1)
            correct_neg = (pred == 0) * (targ == 0)
            res["pos"] = (
                correct_pos[:1].reshape(-1).float().sum(0) * 100.0 / target.sum(0)
            )
            res["neg"] = (
                correct_neg[:1].reshape(-1).float().sum(0)
                * 100.0
                / (batch_size - target.sum(0))
            )
